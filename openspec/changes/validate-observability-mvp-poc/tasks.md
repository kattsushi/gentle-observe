...
| 2B React renderer migration          | 300–400 | Medium | Renderer/lifecycle tests; shell PTY; React/OpenTUI migration; no Atom adapter, Overview, details, or Live.                  |
| 2C Overview + navigation             | 300–420 | Medium | Overview/navigation tests; `--demo` plane-switch/selection harness; UI Overview/layout; no adapter, details, or Live.       |
...
`master ← tracker (draft/no-merge) ← PR0 ← PR1 ← 📍 PR2A ← PR2B (React) ← PR2C (Overview) ← PR3 ← PR4`; PR5/6 after dual continue. PR0→tracker; later→predecessor. Children include `📍` diagram/context; retarget/rebase polluted diffs.
...
### PR2A / PR2B / PR2C Child Boundaries
...
| PR2B  | Start: PR2A; end: equivalent shell behavior on React/OpenTUI.                         | Renderer/lifecycle tests and both shell PTY modes.                           | React dependency/configuration, renderer, and focused tests. | Atom adapter/compatibility claim, Overview/navigation, details, Live, RDD, GitHub. |
| PR2C  | Start: PR2B; end: 2.1b/2.2b Overview/navigation.                                      | Overview tests; `--demo` Runtime/Processes switch, selection, `Enter`/`Esc`. | 2C projection/AppShell/Overview/layout and focused tests.    | Atom adapter, details, Live, dependencies, RDD, GitHub.                            |
...
- [ ] 2.1r **RED:** preserve the PR2A shell, lazy loading, `q`, and cleanup behavior in renderer/lifecycle tests for the selected React migration child.
- [ ] 2.2r **GREEN:** migrate Solid/OpenTUI to React/OpenTUI without adding or claiming the official Atom adapter.
...
## Later Compatibility Gate — Official Atom Solid

- [ ] 0.2 **GATE/GREEN:** only after a published non-downgrade all-peer-compatible tuple exists, validate and add the official Atom Solid adapter; reject peer overrides, downgrades, legacy/fallback, and unpublished packages.
